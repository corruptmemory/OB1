...
app.all("*", async (c) => {
  ...
  // Claude Desktop connectors and many HTTP clients don't send the
  // Accept: text/event-stream header that StreamableHTTPTransport requires.
  // Patch the request to add it when missing so the transport doesn't reject
  // otherwise-valid calls. Ported from server/index.ts (Supabase flavor) —
  // the kubernetes-deployment version this was forked from is missing this fix.
